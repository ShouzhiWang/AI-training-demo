      ...
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="Main navigation">
          ...
          <button className={view === "learn" ? "nav-item active" : "nav-item"} onClick={() => setView("learn")}>
            <span className="nav-icon">◎</span> Prompting 101
            <span className="nav-pill">3/5</span>
          </button>
        </nav>

        <div className="sidebar-section">
          <div className="section-label"><span>MY PROJECTS</span><button aria-label="New project">＋</button></div>
          ...
          <button className="project-row muted">
            <span className="project-icon lavender">✦</span>
        ...
        <section className="workspace">
          <header className="topbar">
            <div className="title-group">
              <button className="back-button" onClick={() => setView("projects")} aria-label="Back to projects">‹</button>
              <div>
            ...
            <div className="top-actions">
              <button className="icon-button" onClick={() => setShowVersions(!showVersions)} aria-label="Version history">↺</button>
              <button className="button secondary" onClick={() => setShowShare(true)}>Share</button>
              <button className="button primary" onClick={() => setIsPublished(true)}>{isPublished ? "Published ✓" : "Publish"}</button>
            </div>
          </header>
          ...
          <div className="studio-grid">
            <section className="assistant-panel">
              <div className="panel-heading">
                <div><span className="ai-orb">✦</span><strong>Muse</strong><span className="online-dot" /></div>
                <button className="icon-button small" aria-label="New conversation">＋</button>
              </div>
                ...
                {stage === 1 && (
                  <div className="suggestion-block">
                    ...
                    {promptStarters.map((prompt) => (
                      <button key={prompt} onClick={() => sendPrompt(prompt)}>{prompt}<span>↗</span></button>
                    ))}
                  </div>
                )}
                    ...
                    <div className="assistant-message">
                      ...
                      <div className="message-body">
                        ...
                        <ul className="change-list">
                          <li><span>＋</span> 10 points for a correct match</li>
                          <li><span>＋</span> A friendly progress streak</li>
                          <li><span>＋</span> Encouraging, specific feedback</li>
                        </ul>
                        {stage === 2 && <button className="apply-button" onClick={applyChange}><span>✦</span> Apply 3 changes</button>}
                        {stage >= 3 && <div className="applied-note"><span>✓</span><div><strong>Changes applied</strong><small>Saved as version 3 · just now</small></div><button onClick={() => setStudioTab("changes")}>View</button></div>}
                      </div>
                    </div>
                  </>
                )}
              ...
              <div className="composer-wrap">
                <div className="composer">
                  ...
                  <div className="composer-footer">
                    <div><button title="Attach">＋</button><span>Plan first</span><button className="toggle on" aria-label="Plan first enabled"><i /></button></div>
                    <button className="send-button" onClick={() => sendPrompt()} aria-label="Send message">↑</button>
                  </div>
                </div>
            ...
            <section className="canvas-panel">
                ...
                <div className="tab-list">
                  {(["preview", "files", "changes"] as StudioTab[]).map((tab) => (
                    <button key={tab} className={studioTab === tab ? "active" : ""} onClick={() => setStudioTab(tab)}>
                      {tab === "preview" ? "◉" : tab === "files" ? "▱" : "↺"} {tab[0].toUpperCase() + tab.slice(1)}
                      {tab === "changes" && stage >= 3 && <span className="change-badge">3</span>}
                    </button>
                  ))}
                </div>
                <div className="device-actions"><button className="active" aria-label="Desktop preview">▰</button><button aria-label="Mobile preview">▯</button><span /><button aria-label="Refresh">↻</button><button aria-label="Open preview">↗</button></div>
              </div>
                  ...
                  <div className="browser-frame">
                    ...
                    <div className="game">
                      <div className="game-header">
                        ...
                        {stage >= 3 && <div className="game-score"><span>★</span><div><small>SCORE</small><strong>{score}</strong></div><i /><div><small>STREAK</small><strong>{score > 0 ? "1 🔥" : "0"}</strong></div></div>}
                      </div>
              ...
              {studioTab === "files" && (
                <div className="files-view">
                  <div className="files-sidebar">
                    ...
                    <button className="active">⌄ <b>habitat-heroes</b></button>
                    <button>　⌘ app</button>
                    <button>　▣ components</button>
                    <button>　▱ game.tsx</button>
                  ...
                  <div className="code-card">
                    ...
                    <div className="code-explainer"><span>✦</span><p><strong>What this does</strong><br />This keeps track of the player’s score and celebrates every correct match.</p></div>
                  </div>
                </div>
              )}
              ...
              {studioTab === "changes" && (
                <div className="changes-view">
                  <div className="changes-header"><div><span className="github-mark">⌘</span><div><strong>Version history</strong><small>Every change is saved — no Git knowledge needed.</small></div></div><button>Open in GitHub ↗</button></div>
                  <div className="timeline">
                    <div className="timeline-item latest"><i /><div><span><strong>v3 · Add score and streak</strong><b>Current</b></span><p>3 files changed <em>+42</em> <del>−8</del></p><small>Created with Muse · just now</small></div><button>•••</button></div>
                  ...
                  <div className="github-card"><span className="github-mark dark">⌘</span><div><strong>Backed up to GitHub</strong><p>Your work is safely synced to <b>alex-lee/habitat-heroes</b></p></div><span className="sync-status">✓ Synced</span></div>
                </div>
              )}
            </section>
          </div>
          ...
          {showVersions && (
            <div className="popover versions-popover">
              <div><strong>Recent versions</strong><button onClick={() => setShowVersions(false)}>×</button></div>
              ...
              <button className="view-all" onClick={() => { setStudioTab("changes"); setShowVersions(false); }}>View all versions →</button>
            </div>
          )}
          ...
          {showShare && (
            <div className="modal-backdrop" onClick={() => setShowShare(false)}>
              <div className="share-modal" onClick={(event) => event.stopPropagation()}>
                ...
                <div className="link-box"><span>muse.site/p/habitat-heroes</span><button onClick={() => setShowShare(false)}>Copy link</button></div>
                <div className="share-options"><button><span>◉</span><strong>Anyone can play</strong><small>No account needed</small></button><button><span>☵</span><strong>Portfolio page</strong><small>Show your process</small></button></div>
              </div>
            </div>
          )}
          ...
          {isPublished && (
            <div className="publish-toast">
              <span>✓</span><div><strong>Your game is live!</strong><small>habitat-heroes.muse.site</small></div><button onClick={() => setShowShare(true)}>View & share ↗</button><button onClick={() => setIsPublished(false)}>×</button>
            </div>
          )}
        </section>
      )}
    ...
    <section className="page-view learn-view">
      ...
      <div className="lesson-layout">
        <article className="lesson-main">
          ...
          <div className="lesson-actions"><button>← Previous</button><button className="button primary" onClick={onContinue}>Try it in the studio →</button></div>
        </article>
        <aside className="lesson-aside">
          <div className="coach-card"><span className="ai-orb large">✦</span><h3>Muse’s coaching note</h3><p>A prompt is a starting point, not a test. You can improve it as you learn what your project needs.</p></div>
          <div className="course-list"><span>YOUR PATH</span><div className="done">✓ <p><strong>Ideas AI can help with</strong><small>Completed</small></p></div><div className="done">✓ <p><strong>Talk like a designer</strong><small>Completed</small></p></div><div className="current">3 <p><strong>The prompt recipe</strong><small>4 min</small></p></div><div>4 <p><strong>Review AI’s work</strong><small>5 min</small></p></div><div>5 <p><strong>Share and reflect</strong><small>3 min</small></p></div></div>
        </aside>
      </div>
    </section>
  );
...
function ProjectsView({ onOpen }: { onOpen: () => void }) {
  return (
    <section className="page-view projects-view">
      <header className="projects-header"><div><span>Good afternoon, Alex</span><h1>What will you create today?</h1></div><button className="button primary">＋ New project</button></header>
      ...
      <div className="section-title"><div><h2>Start with an idea</h2><p>Muse handles the setup. You focus on what you want to make.</p></div><button>View all templates →</button></div>
      <div className="template-grid">
        ...
        <button><span className="template-visual blue">▥</span><div><strong>Research story</strong><p>Explore a question and share what you find.</p><small>15–20 min</small></div></button>
        <button className="coming"><span className="template-visual green">✦</span><div><strong>Start from scratch</strong><p>Describe anything you can imagine.</p><small>Coming soon</small></div></button>
      </div>
    </section>
  );
}
